...
def call_bedrock(system_prompt: str, user_prompt: str, temperature: float = 0.2) -> str:
...
def generate_initial_function(system_prompt: str) -> str:
    return extract_code_block(
        call_bedrock(system_prompt, "Provide the implementation now.", temperature=0.2)
    )
...
def apply_reflexion(
    ...
    return extract_code_block(
        call_bedrock(reflexion_prompt, reflection_context, temperature=0.2)
    )
