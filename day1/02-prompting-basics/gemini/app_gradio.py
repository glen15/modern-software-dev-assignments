...
def call_gemini(system_prompt: str, user_prompt: str, temperature: float = 0.5) -> str:
...
def run_single(example_key: str, system_prompt: str) -> tuple[str, str]:
    ...
    try:
        ...
        output_without = call_gemini("", example["user_prompt"], example["temperature"])
        time_without = time.time() - start_time

        # 시스템 프롬프트 적용해서 실행
        start_time = time.time()
        output_with = call_gemini(system_prompt, example["user_prompt"], example["temperature"])
...
def run_multiple(example_key: str, system_prompt: str, num_runs: int) -> tuple[str, str]:
    ...
    try:
        ...
        for i in range(int(num_runs)):
            start_time = time.time()
            output = call_gemini(system_prompt, example["user_prompt"], example["temperature"])
...
를 정리해주세요."""
        ...
        start_time = time.time()
        synthesis = call_gemini("여러 분석 결과를 객관적으로 종합해주세요.", synthesis_prompt, 0.3)
    ...
    with gr.Column():
        ...
        if example_key == "self_consistency":
            with gr.Row():
                ...
                with gr.Column():
                    output_right = gr.Textbox(label="종합 분석", lines=12, max_lines=12)
            run_btn.click(
